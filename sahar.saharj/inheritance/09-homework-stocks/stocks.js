...
function Stock(symb, share) {
  this.symbol = symb;
  this.shares = share;
}

Stock.prototype.exchange = 'NYSE';
Stock.prototype.buyShares = function (number) {
  this.shares += number;
};
Stock.prototype.sellShares = function(number) {
  if(this.shares >= number) {
    this.shares -= number;
  } else {
    this.share = 0;
  }
